...
public class ActionLog {
    ...
    private RingBufferByteArray[,]      _actionsPerFrame;
    ...
    public RingBufferByteArray[,]       ActionsPerFrame                 { get { return _actionsPerFrame; } }
    ...
    public ActionLog (int pFrameCapacity, int pActionCapacity, int pActionBufferCapacity) {
        ...
        _actionsPerFrame = new RingBufferByteArray[pFrameCapacity, pActionCapacity];
    }
    ...
    public void Add (ulong pFrame, int pActionId, byte[] pActionData) {
        if (pFrame != _frames[_frames.Head]) {
            ...
            for (int i = 0; i < _actionCapacity; i++) {
                if (_actionsPerFrame[_frames.Head, i].Capacity != _actionBufferCapacity) {
                    _actionsPerFrame[_frames.Head, i].Init(_actionBufferCapacity);
                } else {
                    _actionsPerFrame[_frames.Head, i].Clear();
                }
            }
        }

        _actionsPerFrame[_frames.Head, pActionId].Add(pActionData);
    }
    ...
    public void AddAction (ulong pFrame, int pActionId, byte[] pActionData) {
        for (int i = 0; i < _frames.Count; i++) {
            if (_frames[i] == pFrame) {
                _actionsPerFrame[_frames.TranslateIndex(i), pActionId].Add(pActionData);
                return;
            }
        }
    }
    ...
    /// The multidimensional index can be used on <see cref="ActionsPerFrame"/> to retrieve the action-data.
    /// </summary>
    /// <param name="pActionId">The id of the action.</param>
    ...
    public RingBufferByteArray GetActionsBuffer (int pActionId) {
        return _actionsPerFrame[_frames.Head, pActionId];
    }
    ...
    public RingBufferByteArray GetActionsBuffer (ulong pFrame, int pActionId) {
        for (int i = 0; i < _frames.Count; i++) {
            if (_frames[i] == pFrame) {
                return _actionsPerFrame[_frames.TranslateIndex(i), pActionId];
            }
        }
    ...
    public bool TryGetActionsBuffer (ulong pFrame, int pActionId, out RingBufferByteArray pBuffer) {
        for (int i = 0; i < _frames.Count; i++) {
            if (_frames[i] == pFrame) {
                pBuffer = _actionsPerFrame[_frames.TranslateIndex(i), pActionId];
                return true;
            }
        }
